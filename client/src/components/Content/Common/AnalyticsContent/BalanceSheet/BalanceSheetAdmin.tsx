...
const BalanceSheetAdmin = () => {
  ...
  const requestSwitch = async (currReport: string, dataForRequest: any) => {
    switch (currReport) {
      case "1":
        return await getBalanceAsset(dataForRequest, "admin");
      case "2":
        return await getBalanceLiabilities(dataForRequest, "admin");
      case "3":
        return await getFinancialIndicators(dataForRequest, "admin");
      case "4":
        return await getStructureLiabilities(dataForRequest, "admin");
      case "5":
        return await getSolvencyRatios(dataForRequest, "admin");
      case "6":
        return await getWorkingCapital(dataForRequest, "admin");
    }
  };
  ...
  const exportAnalyticsReport = async () => {
    ...
    await exportAnalytics(Number(currReport) - 1, dataForRequest, "admin");
    setLoading(false);
  };
  ...
  switch (currReportForLoader) {
    ...
    case "5":
      children = (
        <SolvencyRatios data={data?.tab1} loading={loading} isAdmin={true} />
  ...
  const tProps = {
    treeData: analyticsFilter,
    onChange: onFilterChange,
    value: currFilter,
    placeholder: "Выберите категории организаций",
...
export default BalanceSheetAdmin;
